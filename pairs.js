...
function pairs(names) {
  let arrayLenght = names.length;
  let newArray = [];
  for (i = 0; i < parseInt(arrayLenght / 2); i++) {
    let firstRandom = names.getRandom();
    let secondRandom = names.getRandom();
    for (var x = 0; x < names.length; x++) {
      if (names[x] === firstRandom || names[x] == secondRandom) {
        names.splice(x, 1);
      }
      //console.log(arrayLenght);
      //console.log(i);
    }
    newArray[i] = firstRandom + " , " + secondRandom;
